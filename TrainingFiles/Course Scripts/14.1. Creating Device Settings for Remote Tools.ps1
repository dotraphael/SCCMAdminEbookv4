...
Set-CMClientSetting -Name "$ClientSettingsName" -RemoteToolsSettings -AccessLevel FullControl -AllowPermittedViewersToRemoteDesktop $True -AllowRemoteControlOfUnattendedComputer $True -AudibleSignal PlayNoSound -FirewallExceptionProfile Domain -ManageRemoteDesktopSetting $True -ManageSolicitedRemoteAssistance $True -ManageUnsolicitedRemoteAssistance $True -PermittedViewer "CLASSROOM\MECM Remote Tools" -RemoteAssistanceAccessLevel FullControl -RequireAuthentication $False -PromptUserForPermission $True
